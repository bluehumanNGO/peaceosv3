...
describe('core matches the locked spec/ crypto-contract vectors', () => {
  it('computeContentHash reproduces the locked JCS and content_hash exactly', () => {
    ...
    expect(result.contentHash.toString('hex')).toBe(expected.content_hash);
  });

  it('derivePackageId reproduces the locked package_id', () => {
    const { contentHashHex } = computeContentHash(content);
